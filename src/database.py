...
class ThatchDB:
    ...
    def _init_sqlite(self) -> None:
        ...
        if "target_monitor" not in columns:
            try:
                cursor.execute("ALTER TABLE games ADD COLUMN target_monitor TEXT DEFAULT 'default'")
    ...
    def data(self) -> dict:
        ...
        if self._games_cache is None or self._config_cache is None:
            self._load_caches()
        return {
            "global_config": self._config_cache,
    ...
    def _load_caches(self) -> None:
        ...
        # Load games
        cursor.execute("SELECT name, exe, prefix, runner, recipe_id, virtual_desktop, virtual_desktop_res, dpi_scale, target_monitor, sandbox FROM games")
        self._games_cache = {}
    ...
    def get_winetricks_cache_dir(self) -> Path:
        path_str = self._get_config_val("winetricks_cache_dir", str(self.default_winetricks_cache_dir))
        p = Path(path_str)
        p.mkdir(parents=True, exist_ok=True)
        return p
    ...
    def list_existing_prefixes(self) -> list[str]:
        ...
        if not p_dir.exists():
            return []
        return sorted([
            entry.name for entry in p_dir.iterdir() 
            if entry.is_dir() and not entry.name.startswith(".") and entry.name != "temp_zeus_prefix"
        ])
    ...
    def add_game(self, name: str, exe: str, runner: str, prefix: str, recipe_id: str = "default_gaming") -> None:
        ...
        cursor.execute("SELECT virtual_desktop, virtual_desktop_res, dpi_scale, target_monitor, sandbox FROM games WHERE name = ?", (name,))
        row = cursor.fetchone()
