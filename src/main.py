...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                               ...
                               QInputDialog, QDialog, QFormLayout, QRadioButton, 
                               QButtonGroup, QTextEdit, QFrame, QSystemTrayIcon, QMenu,
                               QLineEdit, QCheckBox, QStackedWidget)
...
class ThatchLauncher(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        if len(sys.argv) > 1:
            ...
            if arg_path.exists() and arg_path.suffix.lower() == ".exe":
                QTimer.singleShot(150, lambda: self._on_zeus_requested_prefilled(str(arg_path)))
    ...
    def init_ui(self) -> None:
        ...
        self.chest_details_view.back_requested.connect(lambda: self._on_sidebar_view_changed("chests"))
        ...
        self.chest_details_view.install_dependency_requested.connect(self._on_chest_install_dependency)
        self.chest_details_view.remove_dependency_requested.connect(self._on_chest_remove_dependency)
        ...
        self.chest_details_view.virtual_desktop_changed.connect(self._on_chest_virtual_desktop_changed)
        ...
        self.chest_details_view.link_registry_program_requested.connect(self._on_link_registry_program)
    ...
    def load_winetricks_catalog(self) -> list[dict]:
        ...
        try:
            ...
            if cached:
                ...
                print(f"[WinetricksCatalog] Loaded {len(cached)} verbs instantly from SQLite cache.")
                return cached
    ...
    def _trigger_winetricks_catalog_refresh(self, silent: bool = True) -> None:
        ...
        _enrichment: dict[str, tuple[str, str]] = {
            "dxvk":            ("DXVK (Vulkan Translation)",       "High-performance DirectX 9/10/11 to Vulkan translation layer"),
            "dxvk2":           ("DXVK 2.x (Vulkan Translation)",   "Latest DXVK with improved DX12 support via VKD3D-Proton"),
            "vkd3d":           ("VKD3D (DX12 → Vulkan)",          "DirectX 12 to Vulkan translation layer"),
            "vcrun2015":       ("Visual C++ 2015 Runtime",          "Microsoft Visual C++ 2015 standard runtime libraries"),
            "vcrun2017":       ("Visual C++ 2017 Runtime",          "C++ runtime files for programs built with VC++ 2017"),
            "vcrun2019":       ("Visual C++ 2019 Runtime",          "C++ runtime for modern applications built with MSVC 2019"),
            "vcrun2022":       ("Visual C++ 2022 Runtime",          "Latest Microsoft Visual C++ runtime package"),
            "vcrun2013":       ("Visual C++ 2013 Runtime",          "Standard libraries for applications compiled with VC++ 2013"),
            "dotnet48":        (".NET Framework 4.8",               "Microsoft .NET Framework 4.8 runtime environment"),
            "dotnet6":         (".NET 6 Runtime",                   "Cross-platform .NET 6 runtime for modern Windows apps"),
            "dotnet7":         (".NET 7 Runtime",                   "Cross-platform .NET 7 runtime for modern Windows apps"),
            "d3dx9":           ("DirectX 9 DLLs",                   "Legacy Direct3D 9 runtime files for older software"),
            "d3dcompiler_47":  ("D3D Shader Compiler 47",           "Direct3D shader compiler library (d3dcompiler_47.dll)"),
            "physx":           ("NVIDIA PhysX System",              "NVIDIA hardware-accelerated physics engine runtime"),
            "msxml6":          ("MSXML 6.0 Services",               "Microsoft XML parser runtimes and tools"),
            "corefonts":       ("Microsoft Core Fonts",             "Standard Windows web fonts (Arial, Times New Roman, Courier)"),
            "tahoma":          ("Tahoma Font",                      "TrueType font designed primarily for Windows UI elements"),
            "liberation":      ("Liberation Fonts",                 "Metrically compatible fonts for Arial, Times, Courier"),
            "sandbox":         ("Sandbox Mode",                     "Disable Desktop/Documents folder mappings to keep your system clean"),
            "sound=alsa":      ("ALSA Audio Driver",                "Configures Wine audio system to route through ALSA"),
            "renderer=vulkan": ("Vulkan Renderer",                  "Forces Wine to use the modern Vulkan graphics pipeline"),
            "fontsmooth=rgb":  ("RGB Font Smoothing",               "Enables subpixel RGB font antialiasing inside Wine applications"),
        }
    ...
    def refresh_data(self) -> None:
        ...
        for prefix_name in prefixes:
            ...
            for gname, ginfo in games.items():
                if ginfo.get("prefix") == prefix_name and "Contenedor de Sistema" in ginfo.get("exe", ""):
                    primary_recipe = ginfo.get("recipe_id")
                    break
            if primary_recipe and primary_recipe != "default_gaming":
                for gname, ginfo in games.items():
                    if ginfo.get("prefix") == prefix_name and ginfo.get("recipe_id") == "default_gaming":
        ...
        if self.chest_details_view.prefix_name:
            ...
            registry_programs = self.get_wine_installed_programs(self.chest_details_view.prefix_name)
    ...
    def get_wine_installed_programs(self, prefix_name: str) -> list[dict]:
        ...
        skip_patterns = {
            "wine", "mono", "gecko", "microsoft visual c++",
            "windows", "directx", ".net", "vcredist",
        }
    ...
    def get_wine_env(self, prefix_name: str, recipe_id: str, runner_override: str | None = None) -> tuple[dict[str, str], Path | None]:
        ...
        if runner_path.exists():
            bin_dir = runner_path / "files" / "bin" if (runner_path / "files").exists() else runner_path / "bin"
            lib_dir = runner_path / "files" / "lib" if (runner_path / "files").exists() else runner_path / "lib"
            lib64_dir = runner_path / "files" / "lib64" if (runner_path / "files").exists() else runner_path / "lib64"
    ...
    def _on_chest_wizard_finish(self, name: str, recipe_id: str, runner_name: str, sandbox_enabled: bool) -> None:
        ...
        self.console_dialog = WinetricksConsoleDialog("wineboot (Inicialización de Sistema)", name, self)
        self.console_dialog.setWindowTitle(f"Creating Chest: {name}")
        self.console_dialog.console.append(f":: [THATCH] Bootstrapping isolated WINEPREFIX in {prefix_path}...")
        self.console_dialog.console.append(f":: [THATCH] Running wineboot -u command via '{wine_exe}'...")
        ...
        self.process.finished.connect(lambda exit_code, exit_status, n=name: self._on_chest_init_finished(exit_code, n))
        self.process.errorOccurred.connect(lambda err, n=name: self._on_chest_init_error(err, n))
    ...
    def _on_chest_init_finished(self, exit_code: int, name: str) -> None:
        ...
        if exit_code == 0:
            ...
            if self.console_dialog:
                ...
                self.console_dialog = None
            self.toast.show_message(f"¡Contenedor '{name}' inicializado! Buscando dependencias...")
            ...
            if required_verbs:
                self.toast.show_message(f"Inyectando {len(required_verbs)} componentes de la receta automáticamente...")
            ...
            if self.console_dialog:
                self.console_dialog.console.append(f"\n:: [ERROR] wineboot failed with exit code: {exit_code}")
                self.console_dialog.console.append(":: [THATCH] Check console output or verify runner/libraries compatibility.")
                self.console_dialog.btn_close.setEnabled(True)
            self.toast.show_message(f"Contenedor '{name}' falló al crearse (código {exit_code}).")
    ...
    def _on_chest_run(self, prefix_name: str) -> None:
        ...
        for gname, ginfo in games.items():
            if ginfo.get("prefix") == prefix_name and "Contenedor de Sistema" not in ginfo.get("exe", ""):
    ...
    def _on_chest_terminal(self, prefix_name: str) -> None:
        ...
        if not terminal or not shutil.which(terminal):
            QMessageBox.critical(self, "No Terminal Emulator", "No se detectó el emulador de terminal configurado.")
            return
            
        # Spawn terminal with custom variables
        cmd = []
        if terminal == "gnome-terminal":
            cmd = ["gnome-terminal", "--", "bash", "-c", f"export WINEPREFIX='{env['WINEPREFIX']}'; export PATH='{env['PATH']}'; export LD_LIBRARY_PATH='{env.get('LD_LIBRARY_PATH','')}'; cd '{prefix_path}'; exec bash"]
        elif terminal == "konsole":
            cmd = ["konsole", "-e", "bash", "-c", f"export WINEPREFIX='{env['WINEPREFIX']}'; export PATH='{env['PATH']}'; export LD_LIBRARY_PATH='{env.get('LD_LIBRARY_PATH','')}'; cd '{prefix_path}'; exec bash"]
        elif terminal in ["alacritty", "kitty"]:
            cmd = [terminal, "-e", "bash", "-c", f"export WINEPREFIX='{env['WINEPREFIX']}'; export PATH='{env['PATH']}'; export LD_LIBRARY_PATH='{env.get('LD_LIBRARY_PATH','')}'; cd '{prefix_path}'; exec bash"]
        else:
            cmd = [terminal, "-e", f"WINEPREFIX={env['WINEPREFIX']} PATH={env['PATH']} LD_LIBRARY_PATH={env.get('LD_LIBRARY_PATH','')} bash"]
    ...
    def _on_chest_delete(self, prefix_name: str) -> None:
        ...
        if res == QMessageBox.Yes:
            ...
            self.toast.show_message(f"Chest '{prefix_name}' y todos sus lanzadores eliminados.")
    ...
    def _on_chest_add_program(self, prefix_name: str, exe_path_override: str | None = None) -> None:
        ...
        if ok and name.strip():
            ...
            self.db.add_game(
                name=name.strip(),
                exe=str(exe_path),
                runner=self.db.data["global_config"].get("default_runner") or "Wine del Sistema (/usr/bin/wine)",
                prefix=prefix_name,
                recipe_id=recipe_id
            )
            ...
            self.refresh_data()
            self.toast.show_message(f"¡Programa '{name.strip()}' registrado y lanzador directo de Wine creado!")
    ...
    def generate_launcher(self, prefix_name: str, game_name: str, icon_path: Path | None = None) -> None:
        ...
        # Compile environment
        env, runner_path = self.get_wine_env(prefix_name, game_info.get("recipe_id", "default_gaming"), game_info.get("runner"))
        ...
        try:
            with open(sh_path, "w", encoding="utf-8") as f:
                ...
                if target_monitor != "default":
                    f.write("# Cambiar monitor primario temporalmente para forzar Wine a iniciar ahí\n")
                    f.write("OLD_PRIMARY=$(xrandr --query | grep \" primary\" | cut -d\" \" -f1)\n")
                    f.write(f"TARGET_MONITOR=\"{target_monitor}\"\n")
                    f.write("if [ -n \"$TARGET_MONITOR\" ] && [ \"$TARGET_MONITOR\" != \"$OLD_PRIMARY\" ]; then\n")
                ...
                recipe = self.recipes.get(game_info.get("recipe_id", "default_gaming"), {})
                ...
                if sandbox_enabled:
                    ...
                    f.write("    \n")
                    f.write("    # Restrict Wine default documents folders from pointing to your actual Linux home\n")
                    f.write("    user_dir=\"$WINEPREFIX/drive_c/users/steamuser\"\n")
                    f.write("    if [ -d \"$user_dir\" ]; then\n")
                    f.write("        for folder in \"Desktop\" \"Documents\" \"Downloads\" \"Music\" \"Pictures\" \"Videos\"; do\n")
                ...
                if game_dir.name.lower() in ["x64", "bin64", "bin", "win64", "x86"] and game_dir.parent.name.lower() != "drive_c":
                ...
                if vd_enabled:
                    f.write(f"{wrapper_str}\"{wine_cmd}\" explorer /desktop=Thatch,{vd_res} \"{relative_exe}\" \"$@\"\n")
                else:
        ...
        if shutil.which("wrestool") and shutil.which("icotool"):
            try:
                ...
                if wrestool_result.returncode == 0 and ico_path.exists():
                    icotool_result = subprocess.run(
                        ["icotool", "-x", "-o", str(icons_dir), str(ico_path)],
                        capture_output=True, timeout=10
                    )
                    # icotool creates files like name_32x32.png - find the largest one
                    pngs = sorted(icons_dir.glob(f"{clean_name}*.png"), key=lambda p: p.stat().st_size, reverse=True)
                    if pngs:
    ...
    def remove_launcher(self, prefix_name: str, game_name: str) -> None:
        ...
        desktop_path = Path.home() / ".local" / "share" / "applications" / f"thatch-{prefix_name}-{clean_name}.desktop"
        ...
        for desk_p in self._get_desktop_paths():
            ...
            if target_desk_shortcut.exists():
                try:
                    ...
                    print(f"[Launcher Cleanup] Failed to remove Desktop shortcut from {desk_p.name}: {e}")
    ...
    def _on_chest_remove_program(self, prefix_name: str, game_name: str) -> None:
        ...
        if uninstall_str:
            ...
            game_info = self.db.get_game(game_name)
            recipe_id = game_info.get("recipe_id", "default_gaming") if game_info else "default_gaming"
            env, runner_path = self.get_wine_env(prefix_name, recipe_id, game_info.get("runner") if game_info else None)
        ...
        self.refresh_data()
        self.toast.show_message(f"¡Programa '{game_name}' desinstalado y lanzadores eliminados!")
    ...
    def detect_game_exe(self, prefix_name: str, reg_entry: dict) -> Path | None:
        ...
        if install_location:
            ...
            if loc_path.exists() and loc_path.is_dir():
                ...
                blacklist = [
                    "unins", "uninstall", "setup", "install", "crash",
                    "unitycrashhandler", "vc_redist", "vcredist", "dxwebsetup",
                    "touchup", "repair", "config", "launcher"
                ]
                filtered_exes = []
                for p in exe_files:
    ...
    @Slot(str, str, str)
    def _on_link_registry_program(self, prefix_name: str, reg_id: str, install_location: str) -> None:
        ...
        if not path:
            ...
            start_dir = install_location if (install_location and Path(install_location).exists()) else ""
            if not start_dir:
                start_dir = str(self.db.get_prefixes_dir() / prefix_name / "drive_c")
            
            path, _ = QFileDialog.getOpenFileName(
                self, f"Seleccionar ejecutable de ‘{suggested_name}’",
                start_dir, "Executables (*.exe)"
            )
        ...
        self.generate_launcher(prefix_name, name.strip(), icon_path)
        self.refresh_data()
        self.toast.show_message(f"¡'{name.strip()}' vinculado y lanzador creado con éxito!")
    ...
    def _on_chest_run_program(self, prefix_name: str, game_name: str) -> None:
        ...
        wrappers = detect_performance_wrapper()
        if vd_enabled:
            cmd = wrappers + [wine_cmd, "explorer", f"/desktop=Thatch,{vd_res}", relative_exe]
        else:
            ...
            if ginfo.get("prefix") == prefix_name:
                ...
                if ginfo.get("exe") and ginfo.get("exe") != "Contenedor de Sistema":
                    ...
                    if Path(ginfo["exe"]).exists():
                        icon_path = self._extract_exe_icon(prefix_name, Path(ginfo["exe"]), gname)
                    self.generate_launcher(prefix_name, gname, icon_path)
                    
        if updated:
            self.db.save()
            self._sync_recipe_change(prefix_name, "recommended_runner", runner_name)
            self.toast.show_message("Configuración de runner guardada y lanzadores regenerados.")

    def _sync_recipe_change(self, prefix_name: str, key: str, value: any, subkey: str | None = None) -> None:
        ...
        if recipe_path.exists():
            try:
                ...
                print(f"[Sync Error] Failed to synchronize changes to {recipe_path.name}: {e}")
    ...
    def _add_verb_to_recipe(self, prefix_name: str, verb: str) -> None:
        ...
        if recipe_path.exists():
            try:
                ...
                print(f"[Sync Error] Failed to add verb {verb} to {recipe_path.name}: {e}")
    ...
    def _remove_verb_from_recipe(self, prefix_name: str, verb: str) -> None:
        ...
        if recipe_path.exists():
            try:
                ...
                print(f"[Sync Error] Failed to remove verb {verb} from {recipe_path.name}: {e}")

    @Slot(str, bool, bool, bool)
    def _on_chest_perf_settings_changed(self, prefix_name: str, esync: bool, fsync: bool, sandbox: bool) -> None:
        self._sync_recipe_change(prefix_name, "performance_env", "1" if esync else "0", "WINEESYNC")
        self._sync_recipe_change(prefix_name, "performance_env", "1" if fsync else "0", "WINEMFSYNC")
    ...
    @Slot(str, bool, str)
    def _on_chest_virtual_desktop_changed(self, prefix_name: str, enabled: bool, resolution: str) -> None:
    ...
    def _on_chest_dpi_scale_changed(self, prefix_name: str, dpi: int) -> None:
        ...
        recipe_id = associated_game.get("recipe_id", "default_gaming") if associated_game else "default_gaming"
        env, runner_path = self.get_wine_env(prefix_name, recipe_id, associated_game.get("runner") if associated_game else None)
        ...
        try:
            ...
            sp.run(
                ...
                env=env, stdout=sp.DEVNULL, stderr=sp.DEVNULL, timeout=10
            )
            label = {96: "100%", 120: "125%", 144: "150%", 192: "200%"}.get(dpi, f"{dpi} DPI")
            self.toast.show_message(f"Escalado DPI aplicado al cofre: {dpi} DPI ({label}).")
        except Exception as e:
            self.toast.show_message(f"DPI guardado — se aplicará en el próximo inicio de Wine.")
    ...
    def _on_chest_monitor_changed(self, prefix_name: str, monitor_name: str) -> None:
        ...
        self.toast.show_message(f"Pantalla de lanzamiento guardada: {label}. Lanzadores regenerados.")
    ...
    def _on_chest_run_installer(self, prefix_name: str, installer_path: str) -> None:
        ...
        if d_drive.is_symlink() or d_drive.exists():
            try:
                ...
                pass
        try:
            (self.db.get_prefixes_dir() / prefix_name / "dosdevices").mkdir(parents=True, exist_ok=True)
            d_drive.symlink_to(setup_dir)
        except Exception:
            pass
            
        vd_enabled = bool(associated_game.get("virtual_desktop", False)) if associated_game else False
        vd_res = associated_game.get("virtual_desktop_res", "1920x1080") if associated_game else "1920x1080"
        ...
        try:
            ...
            self.toast.show_message(f"Instalador iniciado en segundo plano...")
    ...
    def _show_post_installer_dialog(self, prefix_name: str) -> None:
            ...
            for reg in new_programs:
                ...
                if detected_exe:
                    ...
                    if confirm == QMessageBox.Yes:
                        ...
                        icon_path = self._extract_exe_icon(prefix_name, detected_exe, reg["name"])
                        self.generate_launcher(prefix_name, reg["name"], icon_path)
                        linked_any = True
            
            if linked_any:
                self.refresh_data()
                self.toast.show_message("¡Juego detectado vinculado y lanzador creado con éxito!")
                return
                
            # 2. Fallback to manual selection if nothing was auto-linked
            start_dir = str(drive_c) if drive_c.exists() else ""
            path, _ = QFileDialog.getOpenFileName(
                self, "Seleccionar ejecutable del juego instalado",
                start_dir, "Executables (*.exe)"
            )
            if path:
        ...
        btn_later.setStyleSheet("background: transparent; color: #71717a; border: none; font-size: 12px;")
    ...
    def _process_next_winetricks_queue(self) -> None:
        if not hasattr(self, "winetricks_queue") or not self.winetricks_queue:
            self.toast.show_message("¡Inyección incremental del mapa completada con éxito!")
            self.refresh_data()
            return
    ...
    @Slot(str, str)
    def _on_chest_install_dependency(self, prefix_name: str, verb: str, reuse_dialog: bool = False) -> None:
        ...
        sandbox_enabled = bool(associated_game.get("sandbox", False)) if associated_game else False
        if sandbox_enabled:
            prefix_path = self.db.get_prefixes_dir() / prefix_name
    ...
    def _on_winetricks_finished(self, exit_code: int, exit_status: QProcess.ExitStatus) -> None:
        if self.console_dialog:
            ...
            if exit_code == 0:
                ...
                self.refresh_data()
                self.toast.show_message(f"¡Componente '{self.current_verb}' instalado con éxito!")
            else:
                self.toast.show_message(f"Advertencia: falló instalación de '{self.current_verb}'.")
        ...
        if hasattr(self, "winetricks_queue") and self.winetricks_queue:
            next_verb = self.winetricks_queue.pop(0)
            self._on_chest_install_dependency(self.winetricks_queue_prefix, next_verb, reuse_dialog=True)
        else:
            if self.console_dialog:
                self.console_dialog.btn_close.setEnabled(True)
            if hasattr(self, "winetricks_queue_prefix") and self.winetricks_queue_prefix:
                self.toast.show_message("¡Inyección incremental del mapa completada con éxito!")
                self.winetricks_queue_prefix = ""
        ...
        if log_path.exists():
            try:
                ...
                with open(log_path, "w", encoding="utf-8") as f:
                    for line in lines:
                        if not (verb in line and ("w_workaround" in line or "winetricks" in line)):
                            f.write(line)
                ...
                QMessageBox.critical(self, "Error", f"Error al remover dependencia: {e}")

    # ─── CARGO STORE VIEW ACTIONS ──────────────────────────────────────────────

    @Slot(str, str, str)
    def _on_cargo_install_requested(self, app_name: str, recipe_id: str, target_prefix: str) -> None:
        """
        ...
        if not required_verbs:
            self.toast.show_message(f"El mapa '{app_name}' no requiere dependencias de Winetricks.")
            return
        ...
        if not missing_verbs:
            self.toast.show_message(f"¡Todas las dependencias del mapa '{app_name}' ya están instaladas!")
            return
            
        # Confirm and queue missing verbs
        confirm = QMessageBox.question(
            self, "Aplicar Mapa del Tesoro",
    ...
    def _on_cargo_installation_finished(self, game_name: str, exe_path: str, prefix_name: str, recipe_id: str, auto_install: bool) -> None:
