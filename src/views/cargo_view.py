...
class MapCard(QFrame):
    ...
    def __init__(self, recipe_id: str, recipe_data: dict, parent: QWidget | None = None) -> None:
        ...
        # Description
        lbl_desc = QLabel(recipe_data.get("description", "Receta JSON autoportante para compartir."))
...
class MapasView(QWidget):
    ...
    def _render_grid(self) -> None:
        ...
        for recipe_id, recipe in self.recipes.items():
            ...
            # Filter matches
            match_search = not search_query or search_query in display_name.lower() or search_query in desc.lower()
            if match_search:
    ...
    def _on_install_clicked(self, app_name: str, recipe_id: str) -> None:
        ...
        item, ok = QInputDialog.getItem(
            self, "Seleccionar Cofre Destino",
            f"Selecciona el cofre para aplicar el mapa '{app_name}':",
            self.active_prefixes, 0, False
        )
        if ok and item:
