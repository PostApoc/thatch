                ...
                with urllib.request.urlopen(ge_req, timeout=5) as ge_res:
                    ...
                    if ge_tag:
                        ...
                        for asset in ge_data.get("assets", []):
                            ...
                            if asset_name.endswith(".tar.gz") and not asset_name.endswith(".sha512sum"):
...
class RunnerDownloadWorker(QThread):
    ...
    def __init__(self, name: str, dest_dir: Path, custom_url: str | None = None) -> None:
        super().__init__()
        self.name = name
    ...
    def run(self) -> None:
        try:
            ...
            if self.custom_url:
                ...
                self.status.emit(f"Preparando descarga de GE-Proton...")
            else:
                ...
                if not download_url:
                    ...
                    elif "wine-ge" in self.name.lower() or "ge-proton" in self.name.lower():
                        tag_name = self.name.replace("wine-ge-proton", "GE-Proton").replace("wine-ge-", "GE-Proton")
...
class WineRunnersView(QWidget):
    ...
    def __init__(self, db: ThatchDB, runners: list[str], parent: QWidget | None = None) -> None:
        ...
        lbl_run_desc.setStyleSheet("color: #71717a; font-size: 12px; line-height: 18px; margin-bottom: 4px;")
        run_layout.addWidget(lbl_run_desc)
        
        # Active download display row (hidden initially)
        self.down_status_frame = QFrame()
        self.down_status_frame.setStyleSheet("background-color: #121214; border: 1px solid #2d2d34; border-radius: 6px; padding: 10px;")
        status_box = QVBoxLayout(self.down_status_frame)
        status_box.setSpacing(6)
        
        self.lbl_download_status = QLabel("Ready")
        self.lbl_download_status.setStyleSheet("color: #ffffff; font-size: 11px; font-weight: bold;")
    ...
    def _refresh_runners_list(self) -> None:
        ...
        for runner_data in self.available_runners:
            ...
            if r_id.startswith("ge-proton-custom-"):
                ...
                tag_part = r_id.replace("wine-ge-", "").replace("wine-", "").replace("proton", "Proton")
            ...
            row = QFrame()
            row.setStyleSheet("background-color: #121214; border: 1px solid #2d2d34; border-radius: 6px; padding: 12px;")
            ...
            btn_down.clicked.connect(lambda checked=False, r=runner_data: self._start_runner_download(r))
    ...
    def _start_runner_download(self, runner_data: dict) -> None:
        if self.download_worker and self.download_worker.isRunning():
            QMessageBox.warning(self, "Download in Progress", "Another runner download is currently in progress. Please wait.")
            return
    ...
    def _on_download_finished(self, name: str) -> None:
            ...
            for item in runners_dir.iterdir():
                if item.is_dir() and item.name != self.active_download_folder:
                    ...
                    if runner_type == "soda" and folder_name.startswith("soda"):
                        ...
                        is_same_type = True
                    elif runner_type == "lutris" and (folder_name.startswith("lutris") or folder_name.startswith("wine-lutris") or "ge-proton" in folder_name):
                        is_same_type = True
                    ...
                    if is_same_type:
                        try:
                            ...
                            print(f"[Runners Cleanup] Error purging older runner folder {item.name}: {clean_err}")
        ...
        if purged_count > 0:
            self.toast_requested.emit(f"¡Runner '{name}' listo! Se purgaron {purged_count} compilaciones antiguas.")
        else:
    ...
    def _on_download_error(self, err_msg: str) -> None:
        self.down_status_frame.hide()
        self.runners_list_widget.setEnabled(True)
        QMessageBox.critical(self, "Runner Download Error", f"Failed to download or extract Wine runner:\n{err_msg}")
        self.download_worker = None
