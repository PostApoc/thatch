...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QFrame, QLabel, QPushButton, QFormLayout, QComboBox
...
class WorkspaceDetails(QWidget):
    ...
    def __init__(self, active_gpu: str, parent: QWidget | None = None) -> None:
        ...
        self.lbl_env_status.setStyleSheet("font-size: 11px; font-weight: bold; color: #a1a1aa; padding: 2px 0px;")
        h_layout.addWidget(self.lbl_env_status)
        
        self.btn_auto_inject = QPushButton("⚡ Instalar dependencias de receta")
        self.btn_auto_inject.setStyleSheet("background-color: #202024; color: #ffb300; border: 1px solid #ffb300; font-weight: bold; padding: 6px;")
        ...
        self.btn_secondary_action.setStyleSheet("background-color: #202024; color: #ffb300; border: 1px solid #ffb300; font-weight: bold; padding: 10px; font-size: 11px;")
    ...
    def update_for_game(self, game_name: str, game_info: dict, recipe: dict, missing_verbs: list[str], prefixes_dir: Path) -> None:
        ...
        self.lbl_recipe_badge.setText(f"Receta: {recipe.get('display_name', self.current_recipe_id)}")
        ...
        if missing_verbs:
            ...
            self.lbl_env_status.setStyleSheet("font-size: 11px; font-weight: bold; color: #ffb300; padding: 2px 0px;")
            self.btn_auto_inject.setText(f"⚡ Instalar dependencias de receta ({len(missing_verbs)} faltantes)")
            self.btn_auto_inject.show()
        else:
            self.lbl_env_status.setText("✅ Entorno optimizado y listo")
            self.lbl_env_status.setStyleSheet("font-size: 11px; font-weight: bold; color: #00e676; padding: 2px 0px;")
            self.btn_auto_inject.hide()

    def update_for_env(self, env_name: str, recipe: dict, missing_verbs: list[str], prefixes_dir: Path) -> None:
        ...
        if missing_verbs:
            self.lbl_env_status.setText(f"⚠️ Contenedor incompleto (Falta: {', '.join(missing_verbs)})")
            self.lbl_env_status.setStyleSheet("font-size: 11px; font-weight: bold; color: #ffb300; padding: 4px 0px;")
            ...
            self.lbl_env_status.setStyleSheet("font-size: 11px; font-weight: bold; color: #00e676; padding: 4px 0px;")
            self.btn_auto_inject.hide()
