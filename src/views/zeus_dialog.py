...
from PySide6.QtWidgets import (QDialog, QFormLayout, QHBoxLayout, QVBoxLayout, QLabel, QPushButton, 
                             QButtonGroup, QRadioButton, QComboBox, QLineEdit, 
                             QCheckBox, QTextEdit, QMessageBox, QFileDialog, QWidget)
...
class ZeusInstallerDialog(QDialog):
    ...
    """
    installation_succeeded = Signal(str, str, str, str, bool) # name, exe_path, prefix_name, recipe_id, auto_install_verbs

    def __init__(self, db: ThatchDB, recipes: dict, active_gpu: str, prefilled_setup: str | None = None, parent: QWidget | None = None) -> None:
        ...
        self.opt_repack = QCheckBox("Optimizar descompresión y direccionamiento (InnoSetup/Repacks)")
    ...
    def _browse_setup(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "Buscar setup.exe", "", "Instaladores (*setup*.exe *setup*.bin *.exe)")
        if path:
    ...
    def _run_zeus_engine(self) -> None:
        ...
        if setup_path == "Sin seleccionar" or not self.target_folder_name:
            QMessageBox.warning(self, "Falta Información", "Por favor selecciona el instalador y el nombre de la carpeta destino.")
            return
        ...
        if self.opt_repack.isChecked():
            ...
            self.console.append(":: [ZEUS-ENGINE] Optimización de descompresión y direccionamiento de memoria ACTIVADA.")
        else:
            self.console.append(":: [ZEUS-ENGINE] Optimización de descompresión DESACTIVADA (Instalación estándar).")
        ...
        if self.is_isolated:
            ...
            self.console.append(f":: [ZEUS-ENGINE] Modo Aislado. Usando WINEPREFIX temporal: {self.prefix_path}")
        else:
            ...
            self.console.append(f":: [ZEUS-ENGINE] Modo Compartido. Usando WINEPREFIX existente: {self.prefix_path}")
        ...
        try:
            ...
            self.console.append(f":: [ZEUS-ENGINE] Mapeado '{setup_dir}' como Unidad D: virtual (Aislamiento seguro).")
        ...
        self.console.append(":: [ZEUS-ENGINE] IMPORTANTE: Si estás en modo aislado, instala en la ruta por defecto: C:\\Games\\")
    ...
    def _on_finished(self, exit_code: int, exit_status: QProcess.ExitStatus) -> None:
        ...
        if self.is_isolated:
            ...
            if install_dir and install_dir.exists():
                self.console.append(f":: [ZEUS-ENGINE] Carpeta de juego detectada: {install_dir.name}")
                
                final_prefix_dir = self.db.get_prefixes_dir() / self.target_folder_name.replace(" ", "_").lower()
                if final_prefix_dir.exists():
                    shutil.rmtree(final_prefix_dir, ignore_errors=True)
                    
                self.console.append(f":: [ZEUS-ENGINE] Migrando prefijo completo de {self.prefix_path} a: {final_prefix_dir}")
                ...
                try:
                    ...
                    if not migrated_game_path.exists():
                        migrated_game_path = final_prefix_dir / "drive_c" / "GOG Games" / install_dir.name
                    ...
                    if game_exe:
                        self.console.append(f":: [ZEUS-ENGINE] Registrando juego en biblioteca: {self.target_folder_name}")
                        self.db.add_game(
                        ...
                        self.console.append(":: [ZEUS-ENGINE] AVISO: No se detectó ejecutable. Registra el .exe manualmente.")
                        
                    required_verbs = recipe.get("required_verbs", [])
                    auto_install = False
                    if required_verbs:
                        res = QMessageBox.question(self, "Dependencias de Receta", 
                    ...
                    if auto_install:
                        self.installation_succeeded.emit(self.target_folder_name, str(game_exe or ""), final_prefix_dir.name, self.recipe_id, True)
                    else:
                        QMessageBox.information(self, "Éxito", f"¡Juego '{self.target_folder_name}' instalado y registrado con éxito!")
                        self.installation_succeeded.emit(self.target_folder_name, str(game_exe or ""), final_prefix_dir.name, self.recipe_id, False)
                    self.accept()
                except Exception as e:
                    self.console.append(f"❌ [ZEUS-ENGINE] Error durante migración: {e}")
                    QMessageBox.critical(self, "Error", f"Fallo al migrar archivos: {e}")
            else:
                self.console.append("❌ [ZEUS-ENGINE] AVISO: No se detectó carpeta instalada. ¿Se canceló la instalación?")
                QMessageBox.warning(self, "Cancelado", "Instalación no completada. Prefijo temporal preservado para inspección.")
        else:
            # Existing Shared Prefix flow
            self.console.append(":: [ZEUS-ENGINE] Escaneando prefijo compartido para detectar nueva instalación...")
            after_subdirs = set()
            ...
            if install_dir and install_dir.exists():
                self.console.append(f":: [ZEUS-ENGINE] Nueva carpeta detectada en prefijo compartido: {install_dir.name}")
                ...
                if game_exe:
                    self.console.append(f":: [ZEUS-ENGINE] Registrando juego '{self.target_folder_name}' en biblioteca con exe: {game_exe.name}")
                    self.db.add_game(
                        name=self.target_folder_name,
                        exe=str(game_exe),
                        runner=self.db.get_default_runner() or "Wine del Sistema (/usr/bin/wine)",
                ...
                if auto_install:
                    self.installation_succeeded.emit(self.target_folder_name, str(game_exe or ""), self.prefix_path.name, self.recipe_id, True)
                else:
                    QMessageBox.information(self, "Éxito", f"¡Juego '{self.target_folder_name}' registrado con éxito en prefijo compartido!")
                    self.installation_succeeded.emit(self.target_folder_name, str(game_exe or ""), self.prefix_path.name, self.recipe_id, False)
                self.accept()
            else:
                self.console.append("❌ [ZEUS-ENGINE] AVISO: No se detectó ninguna nueva carpeta de instalación.")
                QMessageBox.warning(self, "Aviso", "No se detectó una carpeta nueva en C:\\Games. Si la instalación tuvo éxito en otra ruta, añade el juego manualmente usando 'Reclutar Juego'.")
                self.accept()
        ...
        for prefix in prefixes_to_lock:
            ...
            if sandbox_enabled:
                ...
                if z_drive.is_symlink() or z_drive.exists():
                    try:
                        z_drive.unlink()
                        self.console.append(f":: [ZEUS-ENGINE] Sandbox re-bloqueado con éxito para: {prefix.name}")
                    except Exception:
                        pass
