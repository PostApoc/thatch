...
from PySide6.QtWidgets import QDialog, QFormLayout, QHBoxLayout, QLabel, QPushButton, QComboBox, QFileDialog, QMessageBox, QWidget
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, db: ThatchDB, parent: QWidget | None = None) -> None:
        ...
        btn_save.setStyleSheet("background-color: #0d5a2d; color: #00e676; border: 1px solid #00e676;")
        btn_save.clicked.connect(self._save_settings)
        layout.addRow(btn_save)

    @Slot()
    def _browse_pref(self) -> None:
        path = QFileDialog.getExistingDirectory(self, "Seleccionar Carpeta de Contenedores", self.lbl_pref.text())
        if path:
            self.lbl_pref.setText(path)

    @Slot()
    def _browse_runners(self) -> None:
        path = QFileDialog.getExistingDirectory(self, "Seleccionar Carpeta de Runners", self.lbl_run.text())
        if path:
            self.lbl_run.setText(path)

    @Slot()
    def _browse_cache(self) -> None:
        path = QFileDialog.getExistingDirectory(self, "Seleccionar Carpeta de Caché", self.lbl_cache.text())
        if path:
    ...
    def _save_settings(self) -> None:
        ...
        self.db.save()
        QMessageBox.information(self, "Ajustes Guardados", "Se ha actualizado la configuración general del ecosistema.")
        self.accept()
